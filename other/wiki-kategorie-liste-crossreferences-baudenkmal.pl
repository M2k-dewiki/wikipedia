...
# https://petscan.wmflabs.org/?&cb_labels_any_l=1&categories=Liste%20(Baudenkm%C3%A4ler%20in%20Bayern)%0A&project=wikipedia&edits%5Banons%5D=both&edits%5Bflagged%5D=both&search_max_results=500&edits%5Bbots%5D=both&cb_labels_no_l=1&language=de&cb_labels_yes_l=1&ns%5B0%5D=1&interface_language=en&show_redirects=no&doit=
...
while(<IN>)
{
    ...
    my $kat = $list;
    $kat =~ s/Liste der Baudenkmäler in /Kategorie\:Baudenkmal in /og;
    $kat =~ s/Liste der Baudenkmäler im /Kategorie\:Baudenkmal im /og;
