...
my $outfile = "out-kategorie-liste-crossreferences-bodendenkmal.txt";
open ( OUT, "> $outfile") || die("cannot open $outfile: $! \n");

my $URL = 'Kategorie:Bodendenkmal in Rohr in Niederbayern';
...
my $INFILE = 'input-list-kategorie-liste-crossreferences-bodendenkmal.txt';
...
while(<IN>)
{
    ...
    my $kat = $list;
    $kat =~ s/Liste der Bodendenkmäler in /Kategorie\:Bodendenkmal in /og;
    $kat =~ s/Liste der Bodendenkmäler im /Kategorie\:Bodendenkmal im /og;
    
    print "LIST:$list:KAT:$kat:\n";   

    $count++; 
    # if ( $count == 2 ) { close(IN); close(OUT); exit ; }    
